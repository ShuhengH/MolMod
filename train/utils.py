...
@torch.no_grad()
def sample(model, x, steps, temperature=1.0, sample=False, top_k=None, top_p=None, prop=None, scaffold=None):
    """
    Modified sampling function that supports both top-k and top-p sampling
    """
    ...
    for k in range(steps):
        ...
        # Apply filters - first top-k if specified
        if top_k is not None:
            logits = top_k_logits(logits, top_k)
            
        # Then apply top-p if specified
